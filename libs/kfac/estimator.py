...
class FisherEstimator(object):
    def __init__(self,
        ...
        setup = self._setup(cov_ema_decay, update_type)
    ...
    def rescale_op(self, scale):
        rescale_ops = [factor.rescale_covariance_op(scale) for factor in self._layers.get_factors()]
        return control_flow_ops.group(*rescale_ops)

    def add_op(self, scale):
        add_ops = [factor.add_covariance_op(scale) for factor in self._layers.get_factors()]
        return control_flow_ops.group(*add_ops)
    ...
    def _setup(self, cov_ema_decay, update_type):
        ...
        cov_updates = [
            factor.make_covariance_update_op(cov_ema_decay, update_type)
            for factor in self._layers.get_factors()
        ]
